...
print (' ---------------------------------------------------------------------')
print (' This is HII-CHI-mistry for UV version 4.0')
   ...
   for monte in range(0,n,1):
      ...
      if Lya_1216[tab] == 0:
         ...
         while Lya_1216_obs <= 0:
            Lya_1216_obs = np.random.normal(Lya_1216[tab],eLya_1216[tab]+1e-3)
      CIV_1549_obs = 0
      if CIV_1549[tab] == 0:
         CIV_1549_obs = 0
      else:
      ...
      if HeII_1640[tab] == 0:
         ...
         if HeII_1640_obs <= 0:
            HeII_1640_obs = np.random.normal(HeII_1640[tab],eHeII_1640[tab]+1e-3)
      OIII_1665_obs = 0
      if OIII_1665[tab] == 0:
         OIII_1665_obs = 0
      else:
         while OIII_1665_obs <= 0:
            OIII_1665_obs = np.random.normal(OIII_1665[tab],eOIII_1665[tab]+1e-3)
      CIII_1909_obs = 0
      if CIII_1909[tab] == 0:
         CIII_1909_obs = 0
      else:
      ...
      if Hb_4861[tab] == 0:
         ...
         while Hb_4861_obs <= 0:
            Hb_4861_obs = np.random.normal(Hb_4861[tab],eHb_4861[tab]+1e-3)
      OIII_5007_obs = 0
      if OIII_5007[tab] == 0:
         OIII_5007_obs = 0
      else:
      ...
      if OIII_1665_obs == 0 or HeII_1640_obs == 0:
         O3He2_obs = 0
      else:
         O3He2_obs = (OIII_1665_obs/HeII_1640_obs)
      ...
      if C3O3_obs == -10:
         ...
         for index in grid:
            ...
            if C3O3_obs == -10:
               ...
               CHI_C3O3 =(np.log10((index[7])/index[6]) - C3O3_obs)**2/np.log10((index[7])/(index[6]+1e-3))
...
# Calculation of O/H and log U
      ...
      if C34_obs == 0 and O3_obs == 0 and ROIII_obs == 0 and C34Hb_obs == 0 and O3Hb_obs == 0 and C34He2_obs == 0 and O3He2_obs == 0 and C3C4_obs == 0:
         OH = 0
         logU = 0
      else:
         CHI_ROIII = 0 
         ...
         for index in grid:
            if CO > -10 and np.abs(index[1] - CO) > np.abs(eCO+0.125):
               ...
               if O3_obs == 0:
                  CHI_O3 = 0
               elif index[6] == 0 or index[3] == 0:
                  CHI_O3 = tol_max
               else:
                  CHI_O3 = (index[6]/index[3] - O3_obs)**2/(index[6]/index[3])
               if O3He2_obs == 0:
                  CHI_O3He2 = 0
               elif index[6] == 0 or index[5] == 0:
                  CHI_O3He2 = tol_max
               else:
                  CHI_O3He2 = (index[6]/index[5] - O3He2_obs)**2/(index[6]/index[5])
               if O3Hb_obs == 0:
                  CHI_O3Hb = 0
               elif index[6] == 0:
                  CHI_O3Hb = tol_max
               else:
                  CHI_O3Hb = (index[6] - O3Hb_obs)**2/index[6]


               if C34Hb_obs > 0:
                  CHI_OH = (CHI_ROIII**2 + CHI_C34Hb**2 + CHI_O3Hb*2 + CHI_C3C4**2)**0.5
               else:	
                  CHI_OH = (CHI_ROIII**2 + CHI_C34**2 + CHI_C34He2**2 + CHI_O3**2 + CHI_O3He2**2 + CHI_C3C4**2 )**0.5
         ...
         for index in grid:
            if CO > -10 and np.abs(index[1] - CO) > np.abs(eCO+res_CO):
               ...
               if C34Hb_obs > 0:
                  ...
                  CHI_OH = (CHI_ROIII**2 + CHI_C34**2 + CHI_C34He2**2 + CHI_O3**2 + CHI_C3C4**2 + CHI_O3He2**2 )**0.5
...
lineas_header = [' HII-CHI-mistry_UV v.4.0 output file', 'Input file:'+input00,'Iterations for MonteCarlo: '+str(n),'Used models: '+sed_type,'','ID.   Lya  eLya   1549   e1549 1640 e1640 1665  e1665  1909   e1909  Hbeta   eHbeta  5007    e5007  i O/H     eO/H  C/O    eC/O  logU   elogU']
