...
print ('-------------------------------------------------')
print (' This is HII-CHI-mistry for UV version 4.21')
...
print ('Hb 4861')
print ('OIII 5007')
print ('in arbitrary units and reddening corrected. Each column must be given with labels and followed by its corresponding flux error.')
print ('-------------------------------------------------')
print (' ')
#Input file reading
if len(sys.argv) == 1:
...
lineas_header = [' HII-CHI-mistry_UV v.4.21 output file', 'Input file:'+input00,'Iterations for MonteCarlo: '+str(n),'Used models: '+sed_type,'Library file used : '+file_lib_2, 'Template used to constraint grid of models: '+const_file,'']
...
#Results
if Label_ID == True: 
   np.savetxt('.'.join(input00.split('.')[:-1])+'_hcm-uv-output.dat',output,fmt=' '.join(['%s']*1+['%.3f']*(len(input1.dtype.names)-1)+['%i']+['%.2f']*6),header=header)
if Label_ID == False: 
   np.savetxt('.'.join(input00.split('.')[:-1])+'_hcm-uv-output.dat',output,fmt=' '.join(['%s']*1+['%.3f']*(len(input1.dtype.names))+['%i']+['%.2f']*6),header=header)
