...
print (' ---------------------------------------------------------------------')
print ('This is HII-CHI-mistry v. 5.2')
...
#FURTHER QUESTIONS REGARDING THE AGN MODEL
if sed == '3':
   #SLOPE ALPHA
   question = True
   while question:
...
#Different library
   ...
   #Counting comments:
   n_comments = 0
   with open('Libraries_opt/'+new_library, 'r') as file12:
      for line in file12:
         if line[0] == '#':
            n_comments += 1  
   grid_aux = np.genfromtxt('Libraries_opt/'+new_library,dtype=None,names=True, encoding = 'ascii', skip_header=n_comments)
   if inter == 0:
...
lin_opt_label = ['12+log(O/H)', 'log(N/O)', 'log(U)', 'OII_3727', 'NeIII_3868', 'OIII_4363', 'OIII_5007', 'NII_6584', 'SII_6717,31']
...
if const == '1' or const == '2' or const == '3' or const == '6':
   ...
   file_open.write('#Library constraitned for '+tag+' ionization AGNs\n')
   #Header line
      ...
      for monte in range(0,n,1):
         ...
         for index in grid:
            ...
            if N2O2_obs == -10:
               ...
               CHI_N2O2 =(np.log10(index['NII_6584']/index['OII_3727']) - N2O2_obs)**2/(abs(np.log10(index['NII_6584']/index['OII_3727'])+1e-3))
            if N2S2_obs == -10: 
               CHI_N2S2 = 0
            elif index['NII_6584'] == 0 or index['SII_671731'] == 0:
               CHI_N2S2 = tol_max
            else:
               CHI_N2S2 =(np.log10(index['NII_6584']/index['SII_671731']) - N2S2_obs)**2/(abs(np.log10(index['NII_6584']/index['SII_671731'])+1e-3))
      ...
      if R23_obs == -10 and NII_6584_obs == 0 and ROIII_obs == 0 and R2Ne3_obs == -10 and O3S2_obs == -10:
         ...
         for index in grid_c:
            ...
            if OII_3727_obs == 0 or OIII_5007_obs == 0:
               ...
               CHI_R23 = (np.log10(index['OII_3727']+index['OIII_5007'])-R23_obs)**2/   (np.abs(np.log10(index['OII_3727']+index['OIII_5007']+1e-3)))
            if OII_3727_obs == 0 or NeIII_3868_obs == 0:
               CHI_O2Ne3 = 0
               CHI_R2Ne3 = 0
               ...
               CHI_R2Ne3 = (np.log10(index['OII_3727']+index['NeIII_3868'])-R2Ne3_obs)**2/   (np.abs(np.log10(index['OII_3727']+index['NeIII_3868']+1e-3)))
            if OIII_5007_obs == 0 or NII_6584_obs == 0:
               CHI_O3N2 = 0
            elif index['OIII_5007'] == 0 or index['NII_6584'] == 0:
               CHI_O3N2 = tol_max
            else:
               CHI_O3N2 = (np.log10(index['OIII_5007']/index['NII_6584']) - O3N2_obs)**2/(np.abs(np.log10(index['OIII_5007']/index['NII_6584']+1e-3)))
            if OIII_5007_obs == 0 or SII_6725_obs == 0:
               CHI_O3S2 = 0
            elif index['OIII_5007'] == 0 or index['SII_671731'] == 0:
               CHI_O3S2 = tol_max
            else:
               CHI_O3S2 = (np.log10(index['OIII_5007']/index['SII_671731']) - O3S2_obs)**2/(np.abs(np.log10(index['OIII_5007']/index['SII_671731']+1e-3)))
...
lineas_header = ['HII-CHI-mistry v.5.2 output file', 'Input file: '+input00,'Iterations for MonteCarlo: '+str(n),'Used models: '+sed_type,'Library file used : '+file_lib_2, 'Template used to constraint grid of models: '+const_file,'']
