# Filename: HII-CHCm_v03.2.py

print ' ---------------------------------------------------------------------'
print 'This is HII-CHI-mistry v. 4.0'
...
print 'Type 0 for missing information.'
print '---------------------------------------------------------------------'
...
while question:
   ...
   print'-------------------------------------------------'
   sed = int(raw_input('Choose SED of the models:'))
   ...
   if inter == 0 and sed==1:
      ...
      print 'No interpolation for the AGN models is going to be used.'
      print 'The grid has a resolution of 0.1dex for O/H and 0.125dex for N/O'
      print ''
      question = False
      res_NO = 0.125
      ...
      print 'Interpolation for the AGN models is going to be used.'
      print 'The grid has a resolution of 0.02dex for O/H and 0.025dex for N/O'
      print ''
      question = False
      res_NO = 0.125
...
# Selection of grid
      ...
      if ROIII_obs > 0:
         grid = grid1
         if monte == n-1: output.append(1+gridbase)
         grid_type = 1
      elif N2O2_obs > -10 or N2S2_obs > -10:
         grid = grid2
         if monte == n-1: output.append(2+gridbase)
         grid_type = 2
      else:
         grid = grid3
         if monte == n-1: output.append(3+gridbase)
         grid_type = 3
...
# Iterations for interpolated models
   ...
   print round(100*(count)/float(len(input)),1),'%',grid_type+gridbase,'', round(OHff,3), round(eOHff,3),'',round(NOff,3), round(eNOff,3), '',round(logUff,3), round(elogUff,3)


out = np.reshape(output,(len(input),19))

np.savetxt('output.dat',out,fmt=' '.join(['%.2f']*12+['%i']+['%.3f']*6))
